...
`0.2.0` 是 Alpha 快照，暂不发布到公共仓库。SwiftUI 与 Compose 已移除旧的运行时字符串组件分发器；ArkUI 改为在生成阶段选择原生控件，不再让所有 API 运行时进入同一个 `switch`；微信小程序输入类组件使用原生控件，Dialog 与 BottomSheet 已补充受控显示、遮罩和关闭事件。
