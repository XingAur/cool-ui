...
Checkbox 属于 actions-inputs 组件，四端共享几何与语义行为，并通过各平台原生能力渲染。
...
```swift
CoolCheckbox(.init(label: "Checkbox", accessibilityLabel: "Checkbox"))
```

```kotlin
CoolCheckbox(props = CoolComponentProps(label = "Checkbox"))
```
