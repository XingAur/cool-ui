...
Slider 属于 actions-inputs 组件，四端共享几何与语义行为，并通过各平台原生能力渲染。
...
```swift
CoolSlider(.init(label: "Slider", accessibilityLabel: "Slider"))
```

```kotlin
CoolSlider(props = CoolComponentProps(label = "Slider"))
```
