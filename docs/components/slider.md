...
Slider is a actions-inputs component. Its contract aligns geometry and semantics while each implementation preserves platform-native behavior.

## API matrix

| SwiftUI | Compose | ArkUI | WeChat |
| --- | --- | --- | --- |
| `CoolSlider` | `CoolSlider` | `CoolSlider` | `<cool-slider>` |
...
```swift
CoolSlider(/* typed platform parameters */)
```
...
```html
<cool-slider label="Slider" accessibility-label="Slider" />
```
