...
ListItem is a content component. Its contract aligns geometry and semantics while each implementation preserves platform-native behavior.

## API matrix

| SwiftUI | Compose | ArkUI | WeChat |
| --- | --- | --- | --- |
| `CoolListItem` | `CoolListItem` | `CoolListItem` | `<cool-list-item>` |
...
```swift
CoolListItem(/* typed platform parameters */)
```
...
```html
<cool-list-item label="ListItem" accessibility-label="ListItem" />
```
