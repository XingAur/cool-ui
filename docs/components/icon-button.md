...
IconButton is a actions-inputs component. Its contract aligns geometry and semantics while each implementation preserves platform-native behavior.

## API matrix

| SwiftUI | Compose | ArkUI | WeChat |
| --- | --- | --- | --- |
| `CoolIconButton` | `CoolIconButton` | `CoolIconButton` | `<cool-icon-button>` |
...
```swift
CoolIconButton(/* typed platform parameters */)
```
...
```html
<cool-icon-button label="IconButton" accessibility-label="IconButton" />
```
