...
TabBar is a navigation component with shared geometry and semantic behavior, rendered through native platform primitives.

## API matrix

| SwiftUI | Compose | ArkUI | WeChat |
| --- | --- | --- | --- |
| `CoolTabBar` | `CoolTabBar` | `CoolTabBar` | `<cool-tab-bar>` |
...
```swift
CoolTabBar(.init(label: "TabBar", accessibilityLabel: "TabBar"))
```

```kotlin
CoolTabBar(props = CoolComponentProps(label = "TabBar"))
```

```html
<cool-tab-bar label="TabBar" accessibility-label="TabBar" />
```
