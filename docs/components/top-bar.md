...
TopBar is a navigation component with shared geometry and semantic behavior, rendered through native platform primitives.

## API matrix

| SwiftUI | Compose | ArkUI | WeChat |
| --- | --- | --- | --- |
| `CoolTopBar` | `CoolTopBar` | `CoolTopBar` | `<cool-top-bar>` |
...
```swift
CoolTopBar(.init(label: "TopBar", accessibilityLabel: "TopBar"))
```

```kotlin
CoolTopBar(props = CoolComponentProps(label = "TopBar"))
```

```html
<cool-top-bar label="TopBar" accessibility-label="TopBar" />
```
