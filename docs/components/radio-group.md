...
RadioGroup is a actions-inputs component with shared geometry and semantic behavior, rendered through native platform primitives.
...
```swift
CoolRadioGroup(.init(label: "RadioGroup", accessibilityLabel: "RadioGroup"))
```

```kotlin
CoolRadioGroup(props = CoolComponentProps(label = "RadioGroup"))
```
