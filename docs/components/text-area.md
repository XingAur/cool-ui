...
TextArea is a actions-inputs component with shared geometry and semantic behavior, rendered through native platform primitives.

## API matrix

| SwiftUI | Compose | ArkUI | WeChat |
| --- | --- | --- | --- |
| `CoolTextArea` | `CoolTextArea` | `CoolTextArea` | `<cool-text-area>` |
...
```swift
CoolTextArea(.init(label: "TextArea", accessibilityLabel: "TextArea"))
```

```kotlin
CoolTextArea(props = CoolComponentProps(label = "TextArea"))
```

```html
<cool-text-area label="TextArea" accessibility-label="TextArea" />
```
