...
Chip is a actions-inputs component with shared geometry and semantic behavior, rendered through native platform primitives.

## API matrix

| SwiftUI | Compose | ArkUI | WeChat |
| --- | --- | --- | --- |
| `CoolChip` | `CoolChip` | `CoolChip` | `<cool-chip>` |
...
```swift
CoolChip(.init(label: "Chip", accessibilityLabel: "Chip"))
```

```kotlin
CoolChip(props = CoolComponentProps(label = "Chip"))
```

```html
<cool-chip label="Chip" accessibility-label="Chip" />
```
