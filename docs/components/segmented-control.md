...
SegmentedControl is a navigation component with shared geometry and semantic behavior, rendered through native platform primitives.

## API matrix

| SwiftUI | Compose | ArkUI | WeChat |
| --- | --- | --- | --- |
| `CoolSegmentedControl` | `CoolSegmentedControl` | `CoolSegmentedControl` | `<cool-segmented-control>` |
...
```swift
CoolSegmentedControl(.init(label: "SegmentedControl", accessibilityLabel: "SegmentedControl"))
```

```kotlin
CoolSegmentedControl(props = CoolComponentProps(label = "SegmentedControl"))
```

```html
<cool-segmented-control label="SegmentedControl" accessibility-label="SegmentedControl" />
```
