...
Button is a actions-inputs component with shared geometry and semantic behavior, rendered through native platform primitives.

## API matrix

| SwiftUI | Compose | ArkUI | WeChat |
| --- | --- | --- | --- |
| `CoolButton` | `CoolButton` | `CoolButton` | `<cool-button>` |
...
```swift
CoolButton(.init(label: "Button", accessibilityLabel: "Button"))
```

```kotlin
CoolButton(props = CoolComponentProps(label = "Button"))
```

```html
<cool-button label="Button" accessibility-label="Button" />
```
