...
Toggle is a actions-inputs component with shared geometry and semantic behavior, rendered through native platform primitives.

## API matrix

| SwiftUI | Compose | ArkUI | WeChat |
| --- | --- | --- | --- |
| `CoolToggle` | `CoolToggle` | `CoolToggle` | `<cool-toggle>` |
...
```swift
CoolToggle(.init(label: "Toggle", accessibilityLabel: "Toggle"))
```

```kotlin
CoolToggle(props = CoolComponentProps(label = "Toggle"))
```

```html
<cool-toggle label="Toggle" accessibility-label="Toggle" />
```
